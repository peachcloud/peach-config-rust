// script to configure a Debian installation to use
...
pub fn configure_networking() -> Result<(), PeachConfigError> {
    ...
    cmd(&["cp", &conf("network/04-wired.network"), "/etc/systemd/network/04-wired.network"])?;
    ...
    cmd(&["cp", &conf("network/copy-wlan.sh"), "/usr/local/bin/copy-wlan.sh"])?;
    cmd(&["chmod", "770", "/usr/local/bin/copy-wlan.sh"])?;
    cmd(&["cp", &conf("network/copy-wlan.service"), "/etc/systemd/system/copy-wlan.service"])?;
    cmd(&["systemctl", "enable", "copy-wlan.service"])?;

    info!("[ SETTING UP WPA_SUPPLICANT AS ACCESS POINT WITH AP0 ]");
    cmd(&["cp", &conf("network/wpa_supplicant-ap0.conf"),
        "/etc/wpa_supplicant/wpa_supplicant-ap0.conf"])?;
    cmd(&["chmod", "600", "/etc/wpa_supplicant/wpa_supplicant-ap0.conf"])?;

    info!("[ CONFIGURING INTERFACES ]");
    cmd(&["cp", &conf("network/08-wlan0.network"), "/etc/systemd/network/08-wlan0.network"])?;
    cmd(&["cp", &conf("network/12-ap0.network"), "/etc/systemd/network/12-ap0.network"])?;

    info!("[ MODIFYING SERVICE FOR ACCESS POINT TO USE AP0 ]");
    cmd(&["systemctl", "disable", "wpa_supplicant@ap0.service"])?;
    cmd(&["cp", &conf("network/wpa_supplicant@ap0.service"),
