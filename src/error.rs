...
pub enum PeachConfigError {
    ...
    CmdIoError { source: std::io::Error, command: String},
    ...
    CmdParseOutputError { source:  std::str::Utf8Error, command: String},
}


impl From<std::io::Error> for PeachConfigError {
    fn from(err: std::io::Error) -> PeachConfigError {
        PeachConfigError::CmdIoError{ source: err, command: "unknown".to_string() }
    }
}
